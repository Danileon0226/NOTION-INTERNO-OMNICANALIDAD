...
export default function AssistantPage() {
  ...
  return (
    <div className="mx-auto flex h-full max-w-3xl flex-col px-4 py-6 sm:px-8 sm:py-8">
      ...
      <div className="flex-1 space-y-3 overflow-y-auto rounded-xl border glass-card p-4">
        {turns.length === 0 && (
            ...
            <div className="mt-3 flex flex-wrap justify-center gap-2">
              {SUGGESTIONS.map((s) => (
                <button
                  key={s}
                  onClick={() => send(s)}
                  className="rounded-full border glass-card px-3 py-1.5 text-xs text-ink hover:bg-bg-subtle"
                >
                  {s}
                </button>
              ))}
            ...
            <div className="max-w-[85%] rounded-2xl bg-bg-subtle px-3.5 py-2 text-sm text-muted">
              ...
              {liveSteps.length > 0 && (
                <div className="mt-1.5 flex flex-wrap gap-1">
                  {liveSteps.map((s, j) => (
                    <span key={j} className="inline-flex items-center gap-1 rounded glass-card px-1.5 py-0.5 text-[10px] text-accent">
                      <Wrench size={10} /> {s.tool}
                    </span>
                  ))}
                </div>
      ...
      <div className="mt-3 flex gap-2">
        <input
          ...
          className="flex-1 rounded-lg border glass-card px-3 py-2 text-sm text-ink outline-none focus:border-accent"
        />
        <button
          onClick={() => send(input)}
