        ...
        <div className="zero-pop fixed bottom-5 right-5 z-40 flex h-[min(620px,calc(100vh-2.5rem))] w-[min(420px,calc(100vw-2.5rem))] flex-col overflow-hidden rounded-xl border bg-bg shadow-2xl">
                ...
                <div className="space-y-1.5">
                  {SUGGESTIONS.map((s) => (
                    <button
                      key={s}
                      onClick={() => send(s)}
                      className="block w-full rounded-lg border bg-white px-3 py-2 text-left text-sm text-ink/90 transition hover:border-accent hover:bg-bg-subtle"
                    >
                      {s}
                    </button>
                  ))}
          ...
          >
            <textarea
              ...
              className="max-h-28 flex-1 resize-none rounded-lg border bg-white px-3 py-2 text-sm text-ink outline-none placeholder:text-muted focus:border-accent"
            />
            <button
              type="submit"
    ...
    <div className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
      <div
        className={`max-w-[85%] whitespace-pre-wrap rounded-2xl px-3.5 py-2 text-sm ${
          isUser ? "bg-ink text-white" : "border bg-white text-ink"
        }`}
      >
        {text}
      </div>
